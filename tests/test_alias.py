...
def test_alias_not_split_for_ru_v_prefix(main_module):
    m = main_module
    alias, rest = m.maybe_split_alias_first_token("в четверг в 20.30 - t2")
    assert alias is None
    assert rest.startswith("в четверг")

def test_alias_not_split_for_month_name_and_day(main_module):
    m = main_module
    alias, rest = m.maybe_split_alias_first_token("25 January 20:30 - t4")
    assert alias is None
    assert rest.startswith("25 january") or rest.startswith("25 January")

def test_alias_split_does_not_trigger_for_on_and_month_names(main_module):
    m = main_module

    alias, rest = m.maybe_split_alias_first_token("on January 25 - t")
    assert alias is None
    assert rest.startswith("on January")

    alias, rest = m.maybe_split_alias_first_token("January 25 - t")
    assert alias is None
    assert rest.startswith("January")
