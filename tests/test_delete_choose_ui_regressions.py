...
def test_delete_choose_del_one_empty_list_edits_no_reminders_and_creates_undo(main_module, monkeypatch):
    ...
    assert reply_text.startswith("Удалил ближайший из серии: ")
    assert "old reminder" in reply_text
...
def test_delete_choose_del_one_rebuilds_remaining_list_and_buttons(main_module, monkeypatch):
    monkeypatch.setattr(main_module, "InlineKeyboardButton", MockButton, raising=False)
    monkeypatch.setattr(main_module, "InlineKeyboardMarkup", MockMarkup, raising=False)
    ...
    assert "old reminder" not in edited_text

    markup = kwargs["reply_markup"]
    buttons = [button for row in markup.keyboard for button in row]
    assert [(button.text, button.callback_data) for button in buttons] == [("❌1", "del:1")]

    assert "undo-token-2" in context.user_data["undo_tokens"]
    assert query.message.replies[0][0].startswith("Удалил ближайший из серии: ")


def test_delete_choose_del_series_empty_list_edits_no_reminders_and_creates_undo(main_module, monkeypatch):
    monkeypatch.setattr(main_module, "InlineKeyboardButton", MockButton, raising=False)
    monkeypatch.setattr(main_module, "InlineKeyboardMarkup", MockMarkup, raising=False)
    ...
    reply_text, kwargs = query.message.replies[0]
    assert reply_text.startswith("Удалил всю серию: ")
    assert "first in series" in reply_text
...
def test_delete_choose_del_index_for_recurring_reminder_shows_delete_mode_choice(main_module, monkeypatch):
    ...
    reply_text, kwargs = query.message.replies[0]
    assert "Это повторяющееся напоминание. Как удалить?" in reply_text
    assert "12.06 10:30 - drink water" in reply_text
    assert "🔁" in reply_text

    buttons = [button for row in kwargs["reply_markup"].keyboard for button in row]
    assert [(button.text, button.callback_data) for button in buttons] == [
        ("🗑 Удалить только ближайший", "del_one:101"),
        ("🧨 Удалить всю серию", "del_series:77"),
    ]
...
def test_delete_choose_del_index_for_regular_reminder_rebuilds_remaining_list(main_module, monkeypatch):
    ...
    reply_text, _kwargs = query.message.replies[0]
    assert reply_text.startswith("Удалил: ")
